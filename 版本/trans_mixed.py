...
def generate_draft(chunk_text, index, total):
    """
    第一步：生成初稿 (Draft)
    """
...
def verify_and_refine_chunk(original_text, draft_translation):
    """
...
def translate_file(file_path, output_path):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        draft = generate_draft(chunk, i, len(chunks))
        
        if draft:
            print(f"    [Step 2/2] 自我核對與修正中... ({i}/{len(chunks)})")
            final_part = verify_and_refine_chunk(chunk, draft)
